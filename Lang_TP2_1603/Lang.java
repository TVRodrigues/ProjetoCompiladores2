...
public class Lang {
      ...
      public static void interpret(LangParser p) throws IOException, Exception {
            try {
                  ...
                  System.out.println("Parsing result: " + presult.value);
                  Node root = (Node) presult.value;// atributo valor do parser,
                  // é em cima dessa raiz que a gente entra com o visitor
                  if (root != null) {
                        Interp v = new Interp();
                        root.accept(v);
                        Object O = v.getStackTop();

                        System.out.println(O == null ? "Null" : O.toString());
                        v.printEnv();
                  } else {
                        System.out.println("Boatos que deu ruim, rejected");

                  }
            } catch (Exception e) {
                  System.out.println("Erro de interpretação: " + e.getMessage());
                  e.printStackTrace();
            }
